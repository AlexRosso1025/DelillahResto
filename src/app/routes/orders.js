...
module.exports = app => {
    app.post('/v1/orders',validateJWT,arrayOrders,validatePaymentMethodOrder,productsExist,insertOrder,insertOrderDetail,(req,res)=>{
        res.status(201).json('new order created');
    });

    app.get('/v1/orders',validateJWT,validateAdmin,async (req,res)=>{
        connection.query(`SELECT o.order_status,o.order_time,o.order_id,o.order_description,group_concat(p.product_name separator ', ') as products,SUM(price) as total_payment, concat(u.firstname, ' ', u.lastname) as fullname,u.address FROM orders o join orders_detail od on o.order_id = od.order_id join products p on od.product_id = p.product_id join users u on o.user_id = u.user_id where o.order_id = od.order_id group by o.order_id`,
